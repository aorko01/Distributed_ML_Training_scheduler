...
# def save_processed_jobs(processed):
...
# # ----------------------
# def find_project_dir(extracted_dir):
...
# # ----------------------
# # Dockerfile generation (simple version)
# # ----------------------
...
#     logger.info("Using base image: python:3.10-slim")
#     return "\n".join(lines)
...
# # Build and push image
# # ----------------------
# def build_and_push(client, job_id, project_dir):
...
#                 if 'stream' in chunk:
#                     print(chunk['stream'], end="")  # live print to terminal
#                 if 'error' in chunk:
#                     logger.error("Build error for job %s: %s", job_id, chunk['error'])
#                     return False
...
# # Notify scheduler
# # ----------------------
# def notify_scheduler_job_ready(job_id):
#     payload = {"job_id": job_id}

#     try:
#         response = requests.post(SCHEDULER_API_URL, json=payload, timeout=10)
#         if response.status_code == 200:
#             logger.info("Scheduler notified for job %s", job_id)
#         else:
#             logger.error(
...
#     logger.info("Watching: %s", UPLOADS_DIR)
